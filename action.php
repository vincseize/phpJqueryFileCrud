...
function get_folder_size($folder_name, $format=true)
{
 $total_size = 0;
...
function get_tot_folders_size($folder)
{
    $folder = array_filter(glob($folder.'/*'), 'is_dir');
    $total_size = 0;
    foreach($folder as $name)
    {
        $total_size = $total_size + get_folder_size($name,false) ;
    }
    return format_folder_size($total_size);
}
...
//   $folder = array_filter(glob('*'), 'is_dir');
  $folder = array_filter(glob($FOLDER_TO_PARSE.'/*'), 'is_dir');
   ...
   foreach($folder as $name)
   {
    $output .= '
     <tr>
      ...
      <td>'.get_folder_size($name).' | '.get_tot_folders_size($FOLDER_TO_PARSE).'</td>
 ...
 {
  $folder = $FOLDER_TO_PARSE.'/'.$_POST["folder_name"];
